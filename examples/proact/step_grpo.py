...
from areal.workflow.step_grpo_workflow import StepGRPOWorkflow


@dataclass
class StepGRPOConfig(GRPOConfig):
    max_turns: int = field(
        default=32,
        metadata={
    ...
    train_num_states_per_gpu: int = field(
        default=1,
        metadata={
            "help": "The number of training states in every actor gpu."
        },
    )
    num_groups_in_parallel_per_gpu: int = field(
        default=1,
        metadata={
            "help": "The number of groups generated in parallel in every actor gpu."
        },
    )
    ...
    use_long_term_value: bool = field(
        default=False,
        metadata={
            "help": "Whether use long term value estimation for each state sample."
        },
    )
    ...
    mask_group_with_same_reward: bool = field(
        default=False,
        metadata={
            "help": "Whether mask group with same reward."
        },
    )
    ...
    use_abs_adv_only_for_group_with_same_reward: bool = field(
        default=False,
        metadata={
            "help": "Whether use absolute advantage only for groups with same reward."
        },
    )
    num_rollouts_in_mc_critic: int = field(
        default=1000,
        metadata={
            "help": "The number of rollout used for monte carlo critic."
        },
    )
    max_steps_in_mc_critic: int = field(
        default=1000,
        metadata={
            "help": "The max steps within a trajectory for monte carlo critic."
        },
    )

def main(args):
    config, _ = load_expr_config(args, StepGRPOConfig)
    ...
    workflow = StepGRPOWorkflow(
        ...
        train_num_states_per_gpu=config.train_num_states_per_gpu,
        num_groups_in_parallel_per_gpu=config.num_groups_in_parallel_per_gpu,
        use_long_term_value=config.use_long_term_value,
        mask_group_with_same_reward=config.mask_group_with_same_reward,
        use_abs_adv_only_for_group_with_same_reward=config.use_abs_adv_only_for_group_with_same_reward,
        num_rollouts_in_mc_critic=config.num_rollouts_in_mc_critic,
        max_steps_in_mc_critic=config.max_steps_in_mc_critic,
    )
    eval_workflow = StepGRPOWorkflow(
    ...
    for global_step in range(start_step, max_steps):
        ...
        with stats_tracker.record_timing("rollout"):
            ...
            # [('input_ids', torch.Size([8, 1063])), 
            # ('loss_mask', torch.Size([8, 1063])), 
            # ('logprobs', torch.Size([8, 1063])), 
            # ('versions', torch.Size([8, 1063])), 
            # ('attention_mask', torch.Size([8, 1063])), 
        ...
        # ('attention_mask', torch.Size([8, 1063])), 
        # ('rewards', torch.Size([8])), 
        # ('prox_logp', torch.Size([8, 1063])), 
        # ('returns', torch.Size([8, 1063])), 
        # ('advantages', torch.Size([8, 1063])), 
        # ('kl_rewards', torch.Size([8, 1063])), 
        # ('tot_rewards', torch.Size([8, 1063]))]
