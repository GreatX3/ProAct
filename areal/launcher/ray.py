...
def run_func(file_path, function_name, *args, **kwargs):
    ...
    if working_dir and os.path.exists(working_dir):
        ...
        try:
            return _run_func_impl(file_path, function_name, *args, **kwargs)
        finally:
...
def _run_func_impl(file_path, function_name, *args, **kwargs):
...
class RayLauncher:
    ...
    def submit(
        ...
        if kwargs is None:
            kwargs = {}
        # Set working directory via environment variable instead of working_dir
        env_vars_with_cwd = (env_vars or dict()).copy()
        env_vars_with_cwd["RAY_WORKING_DIR"] = os.getcwd()
        runtime_env = RuntimeEnv(
            env_vars=env_vars_with_cwd,
        )
        scheduling_strategy = (
