...
class GenerationHyperparameters:
    ...
    n_samples_expert: int = field(
        default=0, metadata={"help": "Number of expert samples in a group."}
    )
...
class PPOActorConfig(TrainEngineConfig):
    ...
    n_epoches: int = field(
        default=1, metadata={"help": "Number of training times for each sample"}
    )
    loss_type: str = field(
        default="ppo",
        metadata={"help": "Loss type, ppo or a2c or a2c_without_log"},
    )
...
class InferenceEngineConfig:
    ...
    train_groups_per_gpu: int = field(
        default=1,
        metadata={
            "help": "The number of train groups in every actor gpu."
        },
    )
