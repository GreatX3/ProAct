...
class WorkflowExecutor:
    ...
    def rollout_batch(
        ...
        if data is not None:
            ...
            batch_size = self.config.train_groups_per_gpu
            ...
            return self.wait(count=batch_size)
    ...
    def prepare_batch(
        ...
        """
        if not hasattr(self, "data_generator") and dataloader is not None:
            self.data_generator = cycle_dataloader(dataloader)
        assert dataloader is None or dataloader.batch_size is not None
        if dataloader is not None:
            batch_size = dataloader.batch_size
        else:
            batch_size = self.config.train_groups_per_gpu
        print(f"prepare_batch batch_size={batch_size}")
        while True:
