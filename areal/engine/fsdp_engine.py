...
class FSDPEngine(BaseHFEngine):
    ...
    def train_batch(
        ...
        print(f"total_loss_weight before reduce={total_loss_weight}")
        dist.all_reduce(total_loss_weight, group=self.dp_group)
        print(f"total_loss_weight after reduce={total_loss_weight}")

        print(f"engine train_batch mb_list: {len(mb_list.padding_lengths)}")
        ...
        for pad_length, padded_mb_input, mb_input in zip(
            ...
            print(f"engine train_batch inputs: {[(k, v.shape if isinstance(v, torch.Tensor) else v) for k, v in inputs.items()]}")
            ...
            print(f"engine train_batch logits: {logits.shape}")
