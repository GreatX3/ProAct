...
logger = logging.getLogger("Step PPO workflow")
...
class StepPPOWorkflow(RolloutWorkflow):
    def __init__(
        self,
        ...
        fix_test_env_seed: bool = True,
        eval_trajs_per_gpu: int = 1,
        turn_gamma: float = 0.99,
    ):
        self.turn_gamma = turn_gamma
        self.eval_trajs_per_gpu = eval_trajs_per_gpu
        self.fix_test_env_seed = fix_test_env_seed
        self.use_lmgame_gamecore = False
        self.use_mc_critic = use_mc_critic
        ...
        self.gamename_to_runepisodefixstep_func = {
            "twenty_forty_eight": self.arun_one_episode_fixstep_turn_independent_2048,
            "sokoban": self.arun_one_episode_fixstep_turn_independent_sokoban,
        }
        ...
        if self.game_name == "twenty_forty_eight":
            self.game_env = TwentyFortyEightVariantEnv(size=4, base=2)
            self.game_env.reset()
        elif self.game_name == "sokoban":
            self.game_env = SokobanVariantEnv(map_file_path=self.sokoban_level_txt_file_path)
            level_idx = random.randint(1, 6)
            self.game_env.reset(level_idx=level_idx)
    ...
    async def arun_one_episode_fixstep_turn_independent_2048(self, engine: InferenceEngine, data):
        seed = data
        system_prompt = self.system_prompt_template
        ...
        step_reward_list = []
        state_value_list = []
        for _ in range(self.train_turns_per_gpu + 1):
            text_obs = self.game_env.get_text_obs()
            user_prompt = self.user_prompt_template.format(textual_representation=text_obs)
            user_messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}]
            ...
            action_dict = self.game_env.get_action_from_response(resp_str.replace("<|im_end|>", ""))
            action_str = None
            if action_dict and action_dict.get("action") is not None:
                action_str = action_dict.get("action")
                action_str = str(action_str).strip().lower()
            
            if self.use_mc_critic:
                game_env_for_cal_value = TwentyFortyEightVariantEnv(size=4, base=2)
                game_env_for_cal_value.reset(seed=seed)
                current_mc_value = game_env_for_cal_value.get_value_for_state(self.game_env.grid, value_calc_way="mean", gamma=self.turn_gamma)
            
            reward, terminated = self.game_env.step(action_str)
            total_score = self.game_env.total_score
            ...
            if not self.use_mc_critic:
                ...
                result = dict(
                    ...
                    rewards=torch.tensor([float(reward)]),
                    terminated=torch.tensor([float(terminated)]),
                    mc_values=torch.tensor([float(current_mc_value)]),
                )
    ...
    async def arun_one_episode_fixstep_turn_independent_sokoban(self, engine: InferenceEngine, data):
        seed = data
        system_prompt = self.system_prompt_template
        ...
        for _ in range(self.train_turns_per_gpu + 1):
            ...
            if self.use_mc_critic:
                current_mc_value = self.game_env.get_value_from_state_sokoban(self.game_env.room_state, gamma=self.turn_gamma)

            symbol_board, reward, terminated, truncated, info_dict, current_perf_score = self.game_env.step(action_str)
            total_score = max(info_dict["total_score"], total_score)
    ...
    async def arun_episode(self, engine: InferenceEngine, data):
        ...
        trajs = await self.gamename_to_runepisodefixstep_func[self.game_name](
                        engine=engine,
                        data=data)
